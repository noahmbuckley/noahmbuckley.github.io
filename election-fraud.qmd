...
Russian survey data is the only source of revealed preferences at any meaningful scale. For each election we assemble individual-level survey reports of regime support — vote recall, or, for some elections, vote intention or approval (the construct varies by election; see the caveat below) — across the major Russian polling organizations (Levada Courier, VTsIOM Sputnik, FOM Georating, RES, PROPA), with up to 7 million respondents across the 12-election window. For each election we fit:
...
The survey-based estimate has now been computed for six elections (Duma 2011/2016/2021, Presidential 2012/2018/2024). A methodological caveat that we state plainly: **the survey quantity is not identical across elections.** For Duma 2011/2016 and Presidential 2012/2018 it is post-election *vote recall* (Levada Courier). For Duma 2021 and the 2020 referendum the only available individual data is *vote intention* (VTsIOM Sputnik), not recall. For Presidential 2024 it is *Putin approval* (PROPA), not a vote-recall item — approval and vote share are related but distinct constructs. We therefore treat the 2024 and intention-based numbers as suggestive rather than as clean vote-gap estimates, and the recall-based elections (2011/2012/2016/2018) as the primary survey evidence. This is disclosed here, in the methodology notes, and in the per-election data provenance.

Caveats: even for the recall-based elections, magnitudes are likely inflated by social-desirability bias (some respondents under-report their Putin/United Russia vote even to a surveyor). The *ordering* across regions is the robust feature; the absolute level should be read as an upper bound. Intention- and approval-based elections (2021, 2024, the 2020 referendum) carry the additional construct caveat above and should not be read as direct vote-fraud magnitudes.
